    ...
    def generate_word_document(self, report_state: ReportState, output_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            if output_path is None:
                ...
                filename = f"거래처분석보고서_{report_state['company_name']}_{timestamp}.docx"
                # 현재 파일 위치를 기준으로 backend/app/output 경로 설정
  ...
  <div class="footer">
    ...
    <p>© 2024 제약사 영업 분석 시스템</p>
        ...
        # 파일 저장
            if output_path is None:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"거래처분석보고서_{report_state['company_name']}_{timestamp}.html"
                # 현재 파일 위치를 기준으로 backend/app/output 경로 설정
                current_dir = Path(__file__).parent
                output_dir = current_dir.parent.parent / "output"
                output_dir.mkdir(parents=True, exist_ok=True)
...
async def run_full_pipeline(agent: ClientAgent,
    ...
    # TextExtractor를 사용하여 사용자 입력 파싱
    text_extractor = TextExtractor()
    extracted_data = text_extractor.extract_company_and_period(user_input)
    
    company_name = extracted_data['COMPANY_NAME']
    start_month = extracted_data['START_MONTH']
    end_month = extracted_data['END_MONTH']
...
if __name__ == "__main__":
    ...
    result = asyncio.run(
        run_full_pipeline(
            ...
            output_dir=None   # None이면 backend/app/output 폴더 사용
        )
    )
