...
def summarize_documents(documents: List[Dict[str, Any]], question: str) -> str:
    ...
    for i, doc in enumerate(documents[:3], 1):  # 상위 3개 문서만 사용
        content = doc.get("content", doc.get("문서내용", ""))
        if content:
...
def question_answering(question: str, top_k: int = 5, include_sources: bool = True) -> Dict[str, Any]:
    ...
    """
    try:
        logger.info(f"QA 처리 시작: {question}")
        ...
        if not keywords:
            return {
                "success": False,
                "question": question,
                "answer": "죄송합니다. 질문에서 검색할 키워드를 추출할 수 없습니다.",
                "summary": None,
                "sources": [],
                "search_results": [],
                "total_sources": 0,
        ...
        # 2. OpenSearch 하이브리드 검색 수행
        search_results = []
        try:
            # OpenSearch 하이브리드 검색 수행
        ...
        if include_sources:
            for i, result in enumerate(search_results, 1):
                ...
                source_info = {
                    "rank": i,
                    "document_name": metadata.get("title", metadata.get("document_name", "N/A")),
