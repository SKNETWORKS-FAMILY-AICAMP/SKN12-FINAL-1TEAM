...
class SalesRecordProcessor(BaseTableProcessor):
    ...
    async def _create_monthly_sales_records(self, row: Dict[str, Any], column_mapping: Dict[str, Any], monthly_sales: List[Dict[str, Any]]) -> List[SalesRecord]:
        ...
        try:
            ...
            logger.debug(f"총 {len(sales_records)}개의 매출 기록 생성 완료")
            self.total_records_created += len(sales_records)  # 총 레코드 수 누적
            return sales_records
...
class BranchProcessor(BaseTableProcessor):
    ...
    async def find_existing_record(self, row: Dict[str, Any], column_mapping: Dict[str, str]):
        ...
        if not branch_name:
                logger.warning("월별 구조를 파악할 수 없습니다. 기본 처리로 전환")
                # 기본 처리로 폴백
                return await super().process_batch(table_data, column_mapping, document_id, uploader_id)
            
            # 2. 데이터 행 처리 (헤더 행 제외)
            data_start_idx = self._find_data_start_index(table_data)
            all_records = []
            skipped_count = 0
            ...
            for row_idx in range(data_start_idx, len(table_data)):
                ...
                # 대상 정보 추출 (첫 번째, 두 번째 컬럼에서)
                target_info = self._extract_target_info(row)
                if not target_info:
                    skipped_count += 1
                    continue
                
                # 지점 및 직원 ID 조회/생성
                branch_id = await self._get_or_create_branch_id_from_info(target_info)
                employee_id = await self._get_or_create_employee_id_from_info(target_info)
                
                if not branch_id or not employee_id:
                    logger.warning(f"행 {row_idx}: 지점/직원 정보 생성 실패")
                    skipped_count += 1
                    continue
                
                # 월별 레코드 생성
                monthly_records = self._create_monthly_records_from_structure(
                    row, monthly_structure, branch_id, employee_id
                )
                
                if monthly_records:
                    all_records.extend(monthly_records)
                    logger.info(f"✅ {target_info.get('name', '알수없음')}: {len(monthly_records)}개 월별 레코드 생성")
            ...
            if all_records:
                ...
                logger.info(f"✅ 총 {len(all_records)}개 branch_targets 레코드 생성 완료")
                return {
                    'success': True,
                    'processed_count': len(table_data) - data_start_idx,
                    'created_count': len(all_records),
                    'updated_count': 0,
            ...
            logger.error(f"❌ branch_targets 배치 처리 중 오류: {e}")
            # 오류 시 기본 처리로 폴백
            return await super().process_batch(table_data, column_mapping, document_id, uploader_id)
    
    def _analyze_table_structure(self, table_data: List[Dict[str, Any]]) -> Dict[str, Dict[str, int]]:
        """테이블 헤더 구조 분석하여 월별 컬럼 매핑 생성"""
        if not table_data:
            return {}
        ...
        # 새로운 컬럼명 형식 처리 (YYYYMM_지표 형식)
        for idx, col in enumerate(columns):
            col_str = str(col)
            
            # YYYYMM_목표, YYYYMM_실적, YYYYMM_달성률 패턴 확인
            if '_' in col_str:
                ...
                if len(parts) == 2 and re.match(r'^\d{6}$', parts[0]):
                    ...
                    if month_str not in monthly_structure:
                        monthly_structure[month_str] = {}
                    
                    if '목표' in metric_type:
                        monthly_structure[month_str]['목표'] = idx
                    elif '실적' in metric_type:
                        monthly_structure[month_str]['실적'] = idx
                    elif '달성' in metric_type:
                        monthly_structure[month_str]['달성률'] = idx
            
            # 단순 YYYYMM 패턴도 처리 (기존 호환성)
            elif re.match(r'^\d{6}$', col_str):
                month_str = col_str
                if month_str not in monthly_structure:
                    monthly_structure[month_str] = {
                        '목표': idx,
                        '실적': idx + 1 if idx + 1 < len(columns) else idx,
                        '달성률': idx + 2 if idx + 2 < len(columns) else idx
                    }
        
        # 구조가 없으면 데이터에서 패턴 찾기 (폴백)
        if not monthly_structure and len(table_data) >= 3:
            header_row1 = table_data[0] if len(table_data) > 0 else {}
            header_row2 = table_data[1] if len(table_data) > 1 else {}
            header_row3 = table_data[2] if len(table_data) > 2 else {}
            
            for idx, col in enumerate(columns):
                val1 = str(header_row1.get(col, '')).strip()
                val2 = str(header_row2.get(col, '')).strip()
                val3 = str(header_row3.get(col, '')).strip()
                
                month_str = None
                if re.match(r'^\d{6}$', val1):
                    month_str = val1
                ...
                if month_str:
                    ...
                    for offset in range(1, 4):
                        if idx + offset < len(columns):
                            next_col = columns[idx + offset]
                            next_val3 = str(header_row3.get(next_col, '')).strip().lower()
                            
                            if '목표' in next_val3:
                                monthly_structure[month_str]['목표'] = idx + offset
                            elif '실적' in next_val3:
                                monthly_structure[month_str]['실적'] = idx + offset
                            elif '달성' in next_val3:
                                monthly_structure[month_str]['달성률'] = idx + offset
        
        if monthly_structure:
            logger.info(f"월별 구조 분석 완료: {len(monthly_structure)}개 월 발견")
            for month, indices in list(monthly_structure.items())[:3]:  # 처음 3개만 로그
                logger.info(f"  {month}: 목표={columns[indices.get('목표', 0)] if indices.get('목표', 0) < len(columns) else 'N/A'}, "
                          f"실적={columns[indices.get('실적', 0)] if indices.get('실적', 0) < len(columns) else 'N/A'}, "
                          f"달성률={columns[indices.get('달성률', 0)] if indices.get('달성률', 0) < len(columns) else 'N/A'}")
        
        return monthly_structure
    
    def _find_data_start_index(self, table_data: List[Dict[str, Any]]) -> int:
        """실제 데이터가 시작되는 행 인덱스 찾기"""
        # 전처리된 데이터는 헤더가 이미 제거되어 있으므로 바로 0부터 시작
        if not table_data:
            return 0
        
        # 첫 번째 컬럼명 확인
        first_col_name = list(table_data[0].keys())[0] if table_data[0] else ""
        
        # 컬럼명이 "대상"이면 전처리된 데이터
        if first_col_name == "대상":
            return 0  # 모든 행이 데이터
        
        # 기존 로직 (전처리되지 않은 경우)
        for idx, row in enumerate(table_data):
            first_col = list(row.keys())[0] if row else None
            first_value = str(row.get(first_col, '')).strip() if first_col else ""
            
            # 헤더가 아닌 실제 데이터 확인
            if first_value and \
               first_value not in ['대상', '구분', '', '목표', '실적', '달성률'] and \
               not re.match(r'^\d{6}$', first_value):  # YYYYMM도 아님
                return idx
        
        return 0  # 기본값: 첫 번째 행부터
    
    def _extract_target_info(self, row: Dict[str, Any]) -> Dict[str, Any]:
        """행에서 대상 정보 추출"""
        columns = list(row.keys())
        if len(columns) < 2:
            return {}
        
        # 첫 번째 컬럼: 대상 (직원명 또는 지점명)
        target_name = str(row.get(columns[0], '')).strip()
        # 두 번째 컬럼: 구분 (지점 또는 부서)
        category = str(row.get(columns[1], '')).strip() if len(columns) > 1 else ""
    ...
    def _create_monthly_records_from_structure(self, row: Dict[str, Any], monthly_structure: Dict[str, Dict[str, int]], 
                                              branch_id: int, employee_id: int) -> List:
        """구조화된 월별 컬럼 정보를 기반으로 레코드 생성"""
                records = []
        columns = list(row.keys())
        
        for month_str, col_indices in monthly_structure.items():
            try:
                ...
                # 목표 값
                if col_indices['목표'] < len(columns):
                    target_val = row.get(columns[col_indices['목표']], '')
                    if target_val and str(target_val) not in ['', 'nan', 'None', '목표']:
                        try:
                            target_amount = float(str(target_val).replace(',', '').replace('%', ''))
                        except (ValueError, TypeError):
                            pass
                
                # 실적 값
                if col_indices['실적'] < len(columns):
                    actual_val = row.get(columns[col_indices['실적']], '')
                    if actual_val and str(actual_val) not in ['', 'nan', 'None', '실적']:
                        try:
                            actual_amount = float(str(actual_val).replace(',', '').replace('%', ''))
                        except (ValueError, TypeError):
                            pass
                
                # 달성률 값
                if col_indices['달성률'] < len(columns):
                    rate_val = row.get(columns[col_indices['달성률']], '')
                    if rate_val and str(rate_val) not in ['', 'nan', 'None', '달성률', '달성']:
                        try:
                            achievement_rate = float(str(rate_val).replace(',', '').replace('%', ''))
                        except (ValueError, TypeError):
                            if target_amount > 0:
                ...
                logger.warning(f"월별 레코드 생성 실패 ({month_str}): {e}")
                continue
        
        return records
    
    async def _get_or_create_branch_id_from_info(self, target_info: Dict[str, Any]) -> Optional[int]:
        """대상 정보에서 지점 ID 조회/생성"""
        try:
            # 카테고리가 지점 정보일 가능성이 높음
            branch_name = target_info.get('category', '') or "본사"
            ...
            logger.error(f"지점 ID 조회/생성 중 오류: {e}")
            return None
    
    async def _get_or_create_employee_id_from_info(self, target_info: Dict[str, Any]) -> Optional[int]:
        """대상 정보에서 직원 ID 조회/생성"""
        try:
            employee_name = target_info.get('name', '')
            ...
            if employee:
                ...
                new_employee = EmployeeInfo(
                    name=employee_name,
                    employee_number=f"AUTO_{employee_name}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                )
            ...
            logger.error(f"직원 ID 조회/생성 중 오류: {e}")
            return None
    ...
    async def create_new_record(self, row: Dict[str, Any], column_mapping: Dict[str, Any]):
        ...
        for monthly_data in monthly_targets:
            try:
                # 달성률 사용 (이미 _extract_monthly_target_data에서 계산됨)
                achievement_rate = monthly_data.get('achievement_rate', 0.0)
    ...
    def _extract_monthly_target_data(self, row: Dict[str, Any], column_mapping: Dict[str, Any]) -> List[Dict[str, Any]]:
        """월별 목표/실적 데이터 추출 - 개선된 버전"""
        monthly_targets = []
        columns = list(row.keys())
        
        # 다단계 헤더 구조를 고려한 데이터 추출
        # 월(YYYYMM) 컬럼을 찾고, 해당 월의 목표/실적/달성률 컬럼 위치 파악
        month_column_map = {}  # {월: {'목표': col_idx, '실적': col_idx, '달성률': col_idx}}
        
        i = 0
        while i < len(columns):
            column_name = columns[i]
            column_value = str(row.get(column_name, '')).strip()
            
            # YYYYMM 패턴 확인 (헤더 행의 월 컬럼)
            if re.match(r'^\d{6}$', column_value):
                month_str = column_value
                month_column_map[month_str] = {}
                
                # 다음 3개 컬럼이 목표/실적/달성률일 가능성 확인
                for j in range(1, 4):
                    if i + j < len(columns):
                        next_col = columns[i + j]
                        next_val = str(row.get(next_col, '')).strip().lower()
                        
                        if '목표' in next_val:
                            month_column_map[month_str]['목표'] = i + j
                        elif '실적' in next_val:
                            month_column_map[month_str]['실적'] = i + j
                        elif '달성' in next_val:
                            month_column_map[month_str]['달성률'] = i + j
                
                # 목표/실적/달성률 컬럼이 없으면 순서대로 할당
                if not month_column_map[month_str]:
                    if i + 1 < len(columns):
                        month_column_map[month_str]['목표'] = i + 1
                    if i + 2 < len(columns):
                        month_column_map[month_str]['실적'] = i + 2
                    if i + 3 < len(columns):
                        month_column_map[month_str]['달성률'] = i + 3
                
                i += 4  # 월 + 목표/실적/달성률 3개 컬럼 건너뛰기
            else:
                i += 1
        
        # 매핑된 컬럼에서 데이터 추출
        for month_str, col_indices in month_column_map.items():
            try:
                year = int(month_str[:4])
                month = int(month_str[4:6])
                ...
                # 달성률 값 추출
                if '달성률' in col_indices:
                    rate_val = row.get(columns[col_indices['달성률']], '')
                    if rate_val and str(rate_val) not in ['', 'nan', 'None', '달성률']:
                        try:
                            achievement_rate = float(str(rate_val).replace(',', '').replace('%', ''))
                        except (ValueError, TypeError):
                            # 달성률 자동 계산
                ...
                logger.warning(f"월별 목표 데이터 파싱 실패: {month_str}, 오류: {e}")
                continue
